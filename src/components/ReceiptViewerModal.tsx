...
export const ReceiptViewerModal: React.FC<ReceiptViewerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/60 p-4 backdrop-blur-xs transition-opacity">
      <div 
        className="relative flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white border border-stone-200 text-stone-900 shadow-2xl"
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-stone-200 px-6 py-4 bg-stone-50">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="rounded-lg p-2 text-stone-400 hover:bg-rose-50 hover:text-rose-600 transition-colors cursor-pointer"
              title="ปิดหน้าต่าง"
            >
              <X className="h-5 w-5" />
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 overflow-y-auto max-h-[calc(90vh-80px)]">
          ...
          <div className="relative col-span-1 md:col-span-2 flex min-h-[350px] md:min-h-[480px] items-center justify-center overflow-hidden bg-stone-100 p-6 border-b md:border-b-0 md:border-r border-stone-200">
            {transaction.slipUrl ? (
              <div 
              ...
              >
                <img
                  ...
                  alt="Receipt slip"
                  className="max-h-[60vh] max-w-full rounded-xl object-contain shadow-lg border border-stone-200"
                  referrerPolicy="no-referrer"
                />
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center text-stone-400 py-12">
                <Tag className="h-12 w-12 mb-3 text-stone-300" />
            ...
            {/* Quick zoom badge */}
            {zoom !== 1 && (
              <div className="absolute bottom-4 left-4 rounded-md bg-white px-2 py-1 text-xs text-stone-700 font-bold border border-stone-200 shadow-2xs">
                {Math.round(zoom * 100)}%
              </div>
            )}
          </div>
          ...
          <div className="col-span-1 p-6 flex flex-col justify-between bg-white">
            <div className="space-y-5">
              ...
              <div className="rounded-xl bg-stone-50 p-4 border border-stone-200">
                ...
                <div className="text-xs text-stone-500 mt-1">
                  หมวดหมู่: <span className="text-stone-900 font-bold">{transaction.category}</span>
                </div>
              </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs text-stone-500 font-semibold">วันที่ทำรายการ</div>
                ...
                <div className="flex items-start gap-3">
                  <CreditCard className="h-4 w-4 text-stone-400 mt-0.5 shrink-0" />
                  <div>
                    <div className="text-xs text-stone-500 font-semibold">วิธีการชำระเงิน</div>
                    <div className="text-stone-900 font-bold">
                ...
                {transaction.vendorOrCustomer && (
                  <div className="flex items-start gap-3">
                    <Building className="h-4 w-4 text-stone-400 mt-0.5 shrink-0" />
                    <div>
                      <div className="text-xs text-stone-500 font-semibold">คู่ค้า / ร้านค้า / ลูกค้า</div>
                      <div className="text-stone-900 font-bold">
                        {transaction.vendorOrCustomer}
                      </div>
                    </div>
                ...
                {transaction.note && (
                  <div className="rounded-xl bg-stone-50 p-3 border border-stone-200 text-xs text-stone-800">
                    <span className="text-stone-500 font-bold block mb-1">บันทึกเพิ่มเติม:</span>
                    {transaction.note}
                  </div>
                )}
              </div>
              ...
              <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50/50 p-4">
                <div className="flex items-center gap-2 mb-2">
                  <QrCode className="h-4 w-4 text-amber-800" />
                  <span className="text-xs font-bold text-amber-950">
                    QR Code ตรวจสอบสลิปต้นฉบับ
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="bg-white p-2 rounded-xl border border-stone-200 shadow-2xs shrink-0">
                    <QRCodeSVG
                      value={qrUrl}
                      size={70}
                      level="M"
            ...
            <div className="pt-4 mt-4 border-t border-stone-200 text-[11px] text-stone-500 flex items-center justify-between">
              ...
              <span className="font-semibold text-emerald-700">สถานะ: คลาวด์ Firebase</span>
            </div>
          </div>
        </div>
      </div>
