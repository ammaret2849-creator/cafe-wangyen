...
export const CafeSettingsModal: React.FC<CafeSettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-xs">
      <div 
        className="relative flex max-h-[90vh] w-full max-w-xl flex-col overflow-hidden rounded-2xl bg-white border border-slate-200 text-slate-900 shadow-2xl"
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-5">
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-2">
              <Building className="h-4 w-4 text-indigo-600" /> ข้อมูลร้านกาแฟ & ตราสัญลักษณ์ (สำหรับหัวรายงานพิมพ์)
            </h4>
            ...
            <div className="flex items-center gap-4 p-3.5 rounded-xl bg-slate-50 border border-slate-200">
              <div className="shrink-0 p-1 bg-white rounded-full shadow-xs border border-slate-200">
                <CollegeLogo size={56} customUrl={logoUrl} />
              </div>
              <div className="flex-1 space-y-1.5 text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-900">ตราสัญลักษณ์วิทยาลัย</span>
                  {logoUrl && (
                    <button
                      type="button"
                      onClick={() => setLogoUrl('')}
                ...
                <div className="flex items-center gap-2 pt-0.5">
                  <label className="cursor-pointer inline-flex items-center gap-1.5 rounded-xl bg-white px-3 py-1.5 text-xs font-bold text-slate-700 border border-slate-300 hover:bg-slate-50 transition-colors shadow-2xs">
          ...
          <div className="space-y-4 pt-4 border-t border-slate-200">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-2">
              <Target className="h-4 w-4 text-indigo-600" /> เป้าหมายทางการเงินประจำเดือน
            </h4>
          ...
          <div className="space-y-3 pt-4 border-t border-slate-200">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-2">
              <Database className="h-4 w-4 text-indigo-600" /> สำรองข้อมูลและกู้คืน (Backup & Restore)
            </h4>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-slate-300 bg-slate-50 px-3.5 py-2 text-xs font-bold text-slate-700 hover:bg-slate-100 transition-colors shadow-2xs"
              >
                <Download className="h-4 w-4 text-indigo-600" />
                สำรองข้อมูลทั้งหมด (JSON)
              </button>

              <label className="flex items-center gap-1.5 rounded-xl border border-slate-300 bg-slate-50 px-3.5 py-2 text-xs font-bold text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer shadow-2xs">
          ...
          {onOpenAuth && (
            <div className="space-y-2 pt-4 border-t border-slate-200">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-2">
                <Shield className="h-4 w-4 text-indigo-600" /> จัดการตำแหน่งและสิทธิ์ผู้ใช้งาน (Staff Accounts)
              </h4>
              <div className="flex items-center justify-between p-3 rounded-xl bg-indigo-50/50 border border-indigo-100">
                <div className="text-xs text-slate-600">
                  <p className="font-bold text-slate-800">จัดการรายชื่อตำแหน่ง, เบอร์โทร และสิทธิ์การเข้าถึง</p>
                  <p className="text-[11px] text-slate-500">สลับบัญชีด่วน, แก้ไขรหัสผ่าน หรือเพิ่ม/ลบตำแหน่งพนักงาน</p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="shrink-0 ml-3 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-2xs transition-colors"
                >
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl px-5 py-2.5 text-sm font-bold text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors"
            >
              ยกเลิก
            </button>
            <button
              type="submit"
              disabled={isSaving}
              className="flex items-center gap-2 rounded-xl bg-slate-900 px-6 py-2.5 text-sm font-bold text-white hover:bg-slate-800 transition-colors shadow-xs disabled:opacity-50"
            >
