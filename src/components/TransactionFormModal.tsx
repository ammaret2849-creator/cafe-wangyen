...
export const TransactionFormModal: React.FC<TransactionFormModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-2 sm:p-4 backdrop-blur-xs overflow-y-auto">
      <div 
        className="relative my-3 sm:my-6 flex max-h-[96vh] w-full max-w-xl flex-col overflow-hidden rounded-xl bg-white border border-slate-200 text-slate-900 shadow-xl"
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                {initialData && (
                  <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-semibold text-slate-700 border border-slate-300">
                    แก้ไข
                  </span>
                )}
              </div>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-4 space-y-3.5 flex-1">
          ...
          {!initialData && (
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 sm:p-2.5">
              <div className="flex items-center gap-1 mb-1.5">
                ...
                <span className="text-[11px] font-bold text-slate-700">รายการด่วน (Presets)</span>
              </div>
              <div className="flex flex-wrap gap-1">
                {QUICK_CAFE_PRESETS.map((preset, idx) => (
                  <button
                    ...
                    className="rounded-md border border-slate-200 bg-white px-2 py-0.5 text-[11px] font-semibold text-slate-700 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-700 transition-colors shadow-2xs"
                  >
                    {preset.label}
                  </button>
                ))}
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400 text-lg font-bold">
                ฿
              </div>
              <input
                type="number"
                ...
                className="w-full rounded-lg border border-slate-300 bg-white py-2 pl-9 pr-3 text-xl font-extrabold text-slate-900 placeholder-slate-400 focus:border-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-600"
                autoFocus
              />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-1 pt-0.5">
              {[50, 100, 300, 500, 1000, 2000, 5000].map((val) => (
                <button
                  ...
                  className="rounded-md border border-slate-200 bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-700 hover:border-slate-300 hover:bg-white transition-colors"
                >
                  +{val.toLocaleString()}
                </button>
              ))}
              {amount && (
                <button
                  type="button"
                  onClick={() => setAmount('')}
                  className="flex items-center gap-0.5 rounded-md border border-slate-200 bg-white px-2 py-0.5 text-[11px] text-slate-500 hover:text-rose-600 hover:border-rose-300 transition-colors"
                >
              ...
              {currentCategories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-lg p-2 text-left text-xs font-semibold transition-all border ${
                      isSelected
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:border-indigo-600 focus:outline-none"
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5 pt-1.5 border-t border-slate-200">
            ...
            {slipUrl ? (
              <div className="relative rounded-lg border border-slate-200 bg-slate-50 p-2.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="relative group cursor-pointer shrink-0" onClick={() => setShowSlipPreviewModal(true)}>
                    <img
                      ...
                      className="h-12 w-12 rounded-md object-cover border border-slate-200 shadow-2xs group-hover:opacity-90 transition-opacity"
                      referrerPolicy="no-referrer"
                    />
                    <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover:opacity-100 rounded-md transition-opacity">
                      <Maximize2 className="h-3.5 w-3.5 text-white" />
                    </div>
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-slate-900 truncate max-w-[180px] sm:max-w-xs">
                ...
                <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center">
                  <button
                    ...
                    className="flex items-center gap-1 rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 rounded-md border border-rose-200 bg-rose-50 px-2 py-1 text-xs font-bold text-rose-600 hover:bg-rose-100 transition-colors"
                    title="ลบสลิปออก"
                  >
              ...
              <div
                ...
                onDrop={handleDrop}
                className={`relative rounded-lg border-2 border-dashed p-3 sm:p-4 text-center transition-all ${
                  dragActive 
              ...
              >
                {isCompressing ? (
                  <div className="py-2 text-indigo-600 text-xs font-bold animate-pulse">
                  ...
                  <div className="flex flex-col items-center justify-center space-y-1.5">
                    ...
                    <div className="flex items-center gap-2 pt-0.5">
                      <button
                        type="button"
                        onClick={() => fileInputRef.current?.click()}
                        className="rounded-lg bg-slate-900 px-2.5 py-1 text-[11px] font-bold text-white hover:bg-slate-800 transition-colors shadow-2xs"
                      >
                        เลือกภาพจากเครื่อง
                      </button>
                      <button
                        ...
                        className="flex items-center gap-1 rounded-lg border border-slate-300 bg-white px-2.5 py-1 text-[11px] font-bold text-slate-700 hover:bg-slate-100 transition-colors shadow-2xs"
                      >
          ...
          <div className="flex items-center justify-end gap-2 pt-2.5 border-t border-slate-200">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 rounded-lg px-4 py-1.5 text-xs font-bold text-white transition-all shadow-2xs ${
                type === 'income'
